function [res,sol] = solve_balanced_connected_sources(TGo, TBack, S, Adj, m, varargin)
% Minimize total absolute deviation from target Tc with connected districts and explicit sources
% (Balanced variant of the min-max model)
...
Tc = sum(min(C, [], 1)) / m;       % target per-salesman score (scalar)
...
% Order: [x(nm), y(nk), v(nmk), g(nm), f(ma), N(m), t(nm), z(m)]
off.x = 0;
off.y = off.x + nm;
off.v = off.y + nk;
off.g = off.v + nmk;
...
obj = zeros(numVars,1);
for r=1:m, obj(idxZ(r)) = 1; end    % minimize sum of deviations

vtype = repmat('B', numVars, 1);
vtype(off.f+1 : off.f+ma) = 'C';              % flows
vtype(off.N+1 : off.N+m)  = 'C';              % N_r
vtype(off.t+1 : off.t+nm) = 'C';              vtype(off.z+1 : off.z+m) = 'C';              % z_r
% t_{ir}
% T
...
% 5) Balanced objective: absolute deviation from Tc using z_r
for r=1:m
    % (i) -z_r + sum_{j,s} C(s,j) * v(j,r,s) <= Tc
    cols = []; vals = [];
    for s=1:k, for j=1:n
        cols(end+1) = idxV(j,r,s); %#ok<AGROW>
        vals(end+1) = C(s,j);      %#ok<AGROW>
    end, end
    cols = [cols, idxZ(r)]; vals = [vals, -1];
    addRow(cols, vals, '<', Tc);

    % (ii) -z_r - sum_{j,s} C(s,j) * v(j,r,s) <= -Tc
    cols = []; vals = [];
    for s=1:k, for j=1:n
        cols(end+1) = idxV(j,r,s); %#ok<AGROW>
        vals(end+1) = -C(s,j);     %#ok<AGROW>
...
if isfield(result,'x')
    ...
    % Balanced info
    sol.z = arrayfun(@(r) x(off.z + r), 1:m);
    sol.Tc = Tc;
% Workloads
    workloads = zeros(1,m);
    useV = (nmk <= 1e6);
    ...
    for i=1:m
        ...
        OutA = ATSP_Im_absdev(Tg+ss, Tb, Tc);
...
function out = ATSP_Im_absdev(G, R, Tc, params)
...
%
%   out = ATSP_Im_absdev(G, R, params)
%   NOTE (modified): objective now minimizes |totalCost - params.TargetCost| by introducing
%   a single scalar deviation variable z with linear constraints.
%   Required: params.TargetCost (Tc).
%
% INPUTS
...
if ~def('TimeLimit'),      params.TimeLimit   = 30;    end
...
if ~def('NoRelHeurTime'),  params.NoRelHeurTime = params.TimeLimit/3; end
...
m1 = n; m2 = n*(n-1); m3 = n; mX = m1+m2+m3; Nvar = mX+n+1; % +1 for deviation variable z
...
idx.xjt = @(j)   m1 + m2 + j;
idx.u   = @(j)   mX + j;
idx.z   = @()    Nvar;  % absolute deviation variable z >= |cost - Tc|

cost_coeff = zeros(Nvar,1);
cost_coeff(1:m1) = alpha(:);
ptr = m1;
for j = 1:n
    for h = 1:n
        if h==j, continue; end
        ptr = ptr + 1;
        cost_coeff(ptr) = C(j,h);
    end
end
cost_coeff(m1+m2+1 : m1+m2+m3) = beta(:);

% New objective: minimize absolute deviation z
obj = zeros(Nvar,1);
obj(idx.z()) = 1;
...
vtype(idx.z()) = 'C'; lb(idx.z()) = 0; ub(idx.z()) = Inf;
...
% Constraints
nRows = 2 + 2*n + n*(n-1) + 2; % +2 rows for absolute deviation constraints
estNnz = 2*n + 2*n*n + 3*n*(n-1) + 2*(mX+1); % reserve for abs-dev rows
...
% Absolute deviation constraints: z >= |cost - Tc|
%   (1)  sum(cost_coeff.*x) - z <= Tc
%   (2) -sum(cost_coeff.*x) - z <= -Tc
cols = find(cost_coeff ~= 0); vals = cost_coeff(cols);
addRow([cols(:); idx.z()], [vals(:); -1], '<', Tc);
addRow([cols(:); idx.z()], [-vals(:); -1], '<', -Tc);
...
params.Cutoff = (1+params.CutoffSlack) * abs(heur_cost - Tc);
    end
end
...
out.cost        = exact_cost;
out.target      = Tc;
out.deviation   = abs(exact_cost - Tc);
out.result      = result;
