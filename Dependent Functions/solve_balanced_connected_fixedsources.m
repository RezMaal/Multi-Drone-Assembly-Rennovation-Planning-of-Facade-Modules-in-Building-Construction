function [res,sol] = solve_balanced_connected_fixedsources(TGo, TBack, S, Adj, m, varargin)
...
off.t = off.N + m;
off.z = off.t + nm;
off.yRS = off.z + m;               % NEW block for yRS (m x k)
numVars = off.yRS + mk;
...
ub(off.yRS+1 : off.yRS+mk) = 1;    % NEW: cap yRS ≤ 1
